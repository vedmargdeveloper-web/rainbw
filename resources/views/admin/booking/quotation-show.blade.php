...
@section('content')
    <div class="main-container">
        ...
        <div class="content-wrapper">
            ...
            <?php
               ...
               $invoice_items =  App\Models\QuotationsItem::where('invoice_id',$inquiry->id)->get();
            ...
            <form action="{{ route('booking.store') }}" method="post" id="invoice-submit"> 
                ...
                <div class="invoice-box"> 
                                        ...
                                        <td colspan="2">
                                            <select class="select-2 main-gst-selection" name="gst_id">
                                                ...
                                                @foreach($gstMaster as $key => $gm)
                                                    <option {{ $key == 0 ? 'selected' : '' }} value="{{ $gm->id }}"> {{ strtoupper($gm->gstin) }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                    ...
                    @php 
                        ...
                        $pitchLastest =  $quotation->pitchLastest ? $quotation->pitchLastest : [];

                        if(!$pitchLastest){
                            dd('Pitch not Created Yet');
                        }
                                ...
                                <tr class="center item">
                                    ...
                                    <td class="item">
                                        <select class="form-control select-2 select-item-product" name="item_id[]" >
                                                ...
                                                @foreach($item as $it)
                                                    <option value="{{ $it->id }}"  {{ ($invoice_item->id==$it->id) ? 'selected' : ''  }} >{{ $it->name }}
                                    ...
                                    <td class="item_rate">
                                        <input class="td-input-width item-gross-total"  type="number" readonly="" name="prate[]" value="{{ $prate[$mykey]  ?? '' }}"  value=""  />
                                    </td>
                                    <td class="item_qty"><input class="td-input-width item-gross-total" readonly=""   type="number"   name="pqty[]"  value="{{ $qty[$mykey]  ?? '' }}" /></td>
                                    <td class="item_pday"> <input class="td-input-width item-gross-total" readonly=""  type="number"   name="pday[]" value="{{ $days[$mykey] ?? '' }}" />
                                    </td>

                                     <td class="gross-amount green">{{ $gross_amount[$mykey] ?? 0 }}
                                                <?php $edit_grand_total += $gross_amount[$mykey] ?>
                                    </td>
                                    <td><input class="td-input-width item-discount"  type="number" name="pdiscount[]" readonly="" value="{{ $discount[$mykey] ?? 0 }}"/></td>
                                    <td class="cgst">{{ $cgst[$mykey] ?? 0 }}</td>
                                        <input type="hidden" class="cgst" name="cgst[]"  value="{{ $cgst[$mykey] ?? 0 }}" />
                                    <td class="sgst">{{ $sgst[$mykey]  ?? 0}}</td>
                                        <input type="hidden" class="sgst" name="sgst[]"  value="{{ $sgst[$mykey]  ?? 0}}" />
                                    <td class="igst">{{ $igst[$mykey] }}</td>
                                        <input type="hidden" class="igst" name="igst[]" value="{{ $igst[$mykey] }}" />
                                    <td class="tax-amount">{{ $tax_amount[$mykey] ?? 0 }}</td>
                                    <td class="total-amount">{{ $total_amount[$mykey] }}
                                    </td>
                                        <input type="hidden" class="pgros_amount" name="pgros_amount[]" value="{{ $gross_amount[$mykey] }}" />
                                        <input type="hidden" class="ptax_amount" name="ptax_amount[]" value="{{ $tax_amount[$mykey] }}" />
                                        <input type="hidden" class="ptotal_amount" name="ptotal_amount[]" value="{{ $total_amount[$mykey] ?? 0 }}" />
 ...
 --}}
                    ...
                    {{-- {{ dd($inquiry) }} --}}
                                <input name="total_gross_sum" id="total_gross_sum" type="hidden" value="{{ $edit_grand_total ?? 0 }}" >
                                <input name="total_tax_amount" id="total_tax_amount" type="hidden" value="{{ $pitchLastest->total_tax_amount ?? 0 }}" >
                                <input name="total_grand_amount" id="total_grand_amount" type="hidden" value="{{ $pitchLastest->total_grand_amount ?? 0 }}" >
                                <input name="total_net_discount" id="total_net_discount" type="hidden" value="{{ $edit_gross_total ?? 0 }}" >
                                   {{--  <input name="amount_in_words" id="amount_in_words" type="hidden" value="{{ $quotation->amount_in_words }}" > --}}
                                <input name="customer_id" id="customer_id" type="hidden" value="{{ $quotation->customer_id }}" >
                                    ...
                                    <tr>
                                        <td class="title center">
                                            <h3>Supply Address</h3>
                                             <select class="select-2 invoice-supply-address" name="supply_id" style="">
            ...
            <script>
                ...
                var myvar =  '<tr class="center item">'+
                    ...
                    '                                    <td ><input type="hidden"  class="w-100" name="venue[]" value="" /></td>'+
                    '                            </tr>';
