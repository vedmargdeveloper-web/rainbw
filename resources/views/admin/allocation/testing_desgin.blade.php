    ...
    <body>
            ...
            <table >
                <thead>
                    <tr>
                        ...
                        <th rowspan="3" class="fixbg fix7">49</th>
                    ...
                    <tr>
                        <th class="fixbg fix7">Booking Qty</th>
                        <th>TT Count</th>
                        <th>Damaged</th>
                        <th>TT Available</th>
                        <th>Booked</th>
                        ...
                        @if($customers->count() > 0 )
	                        @foreach($customers as $customer)
	                        	...
	                        	<?php 
	                        		$qty_th .="<th>Qty</th>";
	                        		$value_td .="<td class='background-dynamic-td td_".$customer->id."' >0</td>";
	                        	?> 
	                        @endforeach
	                    @endif
                  		...
                  		@foreach($booking->bookingItem as $item)
                  			...
                  			@for($ia =0; $ia < $item->days; $ia++)
			                    ...
			                    <tr data-unlocated="{{ $total_tr_unlocated_sum }}"  id="tr_<?= md5($booking->invoice_no.'_day'.$ia.'_item'.$item->id) ?>" class="{{ ($allocation_db_value > 0 ) ? 'color-red' : 'allocated'   }}">
			                        ...
			                        <td class="booking_id fix3" style="background:{{ $unique_color }}"> {{ $booking->invoice_no ?? 'Somthing wrong' }} </td>
        ...
        <div class="bottom-tables">
            ...
            <div class="right" style="width: 69%;">
                <table class="allocation-table" style="width: 45%; margin: 10px 0;">
                    <thead>
                        <tr>
                            <th>Booking ID</th>
        ...
        <script type="text/javascript">
        	...
        	$('input[name="select_booking"]').click(function(){
                   ...
                   // return;
                    $(grand_unallocated_booking).text(global_get_requred_qty-dynamic_td_input);
			...
			$("body").on('click','#btn-allocation',function(event){
                    //$("#btn-allocation").click(function(event){
