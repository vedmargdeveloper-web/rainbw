            ...
            <form action="{{ route('invoice.update',['invoice'=>$invoice->id]) }}" method="post" id="invoice-submit">
                ...
                <div class="invoice-box-">
                  <div class="invoce-table-bord">
                    ...
                    <table class="billing-information" style="width: 100%; table-layout: fixed;">
                      <tbody>
                          <tr>
                              ...
                              {{-- <td>UID:</td>
                              <td colspan="2">  @foreach($inquiry as $inq)
                                            @if($invoice->enquire_id == $inq->id)
                                                {{ $inq->unique_id }}
                                            @endif
                                         @endforeach</td> --}}
                              <td colspan="6"></td>
                              <td style="background: unset;border: unset; width: 100%; display: block; text-align:center">Prf Inv Date:</td>
                    ...
                    @php 
                       ...
                       // dd($pitchLastest);
                        ...
                        foreach ($item_id as $key) {
                            $item_list_selected = $item_list_selected->merge($item_list->where('id', $key));
                        }
                        ...
                        <thead>   
                            ...
                            @endphp
                                ...
                                <tr class="item">
                                            ...
                                            <td class="gross-amount green">{{ round($gross_amount[$mykey])  ??  ''}}
                                                <?php $edit_grand_total += $gross_amount[$mykey] ?>
                                            </td>
                                            <td class="green">{{ $discount[$mykey] }}</td>

                                            <td class="cgst green">{{ 0 }}</td>
                                            <td class="sgst green">{{ 0 }}</td>
                                            <td class="igst green">{{ 0 }}</td>
                                            <td class="tax-amount green">{{  0 }}</td>
                                            <td class="total-amount ">{{ $gross_amount[$mykey] ?? '' }}</td>
                            ...
                            <tr class="center bottom-footer-tr">
                              ...
                              <tr>
                                ...
                                <td colspan="11" class="green" style="font-weight: 600; text-align: left;"><span id="amountinword">{{ getIndianCurrency($edit_grand_total) }}</span></td>
                            </tr>
                              <tr>
                    ...
                    <center> <p class="center" style="padding-bottom: 10px;">This is a system generated Proforma Invoice and does not require a signature.</p></center>
                </div>
                </div>
            </form>
