...
$("#inquiry_unique_id").change(function () {
    ...
    window.location = base_url + "/admin/inquiry/" + $(this).val() + "/edit";
        ...
        $(document).ready(function () {
            $("#change_billing_type").html(capitalize($("#challan_type").val()));
        });
       $("#challan_type").change(function () {
            var code = $(this).val().substring(0, 2).toUpperCase();
            var default_invoice_no = $("#default-challan-no").text();
            $("#footer_challan_type").html($(this).val());
            $("#change_billing_type").html(capitalize($(this).val()));

            // Ajax request to get totalInvoice
            $.ajax({
                type: 'get',
                url: 'http://localhost/rainbow/admin/challan/create',
                data: {
                    challan_type: $(this).val()
                },
                success: function (response) {
                    // Display the totalInvoice count in the specified <span> element
                    $("#total-invoice-count").text(response.totalChallan);
                    
                    // Append the totalInvoice count to the invoice_no variable
                    final_code_with_count = default_challan_no.replace("X", code).trim() + (response.totalChallan + 1);
...
$("body").on("change", ".select-item-product", function () {
    ...
    var input_pfromdate = $(this).closest("tr").find("input.pfrom_date");
    var input_ptodate = $(this).closest("tr").find("input.pto_date");
