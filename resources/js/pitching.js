...
//     url = baseUrl + "/admin/fetch/customers/details/" + user_id;
//     $.ajax({
//         url: url,
//         type: "GET",
//         dataType: "json",
...
$(".invoice-supply-address").change(function () {
    ...
    $.ajax({
        ...
        .done(function (success) {
            ...
            $("#supplyaddress").html(
                `${success.address}  ${success.city.city} ${success.state.state}  ${success.pincode}  ${success.landmark == null ? "" : success.landmark} <br/> ${success.contact_person_name == null ? "" : success.contact_person_name}  `
            );
        })
        .fail(function () {
            console.log("error");
...
$("body").on("focusout", ".item-gross-total", function () {
    ...
    var _current_tr_ = $(this).parents().closest("tr");

    $(".select-item-product").change();
    /* Act on the event */
    var div_gross_total = _current_tr_.children("td.gross-amount");
    var input_gross_total = _current_tr_.children("input.pgros_amount");
    var input_ptax_amount = _current_tr_.children("input.ptax_amount");
    var item_discount_current = _current_tr_.children('td').children('input.td-input-width.item-discount').val();
    // console.log('item_discount_current',);
    var div_tax_amount = _current_tr_.children("td.tax-amount");

    var div_total_amount = _current_tr_.children("td.total-amount");
    ...
    var input_product_total_amount = _current_tr_.children("input.ptotal_amount");
    ...
    // console.log('dxddsfsdf',total_gross_amount);
    // return ;
    //console.log('globale',Global_tax_apply);

    var tax_amount = (parseInt(total_gross_amount) * parseInt(Global_tax_apply) ) / 100;
    ...
    var grand_total = parseFloat(total_gross_amount) + parseFloat(tax_amount);
    ...
    $(input_product_total_amount).val(grand_total);
    //$("input[name='revised_quote']").val(grand_total.toFixed(2));
...
$("body").on("focusout", ".pitching-item-gross-total", function () {
    ...
    $(".select-item-product").change();
    /* Act on the event */
    var _form_div_current =$(this).parents().closest('form');
    var _tax_amount =  _form_div_current.children('input.total_tax_amount');
    
    var _total_grand_amount =  _form_div_current.children('input.total_grand_amount');
    var _total_net_amount =  _form_div_current.children('input.total_net_discount');
    var _total_gross_amount =  _form_div_current.children('input.total_gross_amount');
    //console.log(_total_grand_amount);
    //var _this_total_amount = $(this).parents().closest('tr').children('td.total-amount');
    
    // var _this_total_amount = $(this).parents().closest('tr').children('td.total-amount');
    var _data_key = $(this).parents('tr').closest('tr.center.item').attr('data-key');
    var _data_pitch = $(this).parents('tr').closest('tr.center.item').attr('data-pitch');
    ...
    var div_total_amount = $(this).parents().closest("tr").children("td.total-amount");
      console.log(" div_total_amount", div_total_amount);
    var this_item_rate = $(this).parent().closest("tr").children("td.item_rate").children("input.td-input-width").val();
    ...
    $(input_product_total_amount).val(grand_total);
    //revised_quote
    console.log('Pitching Testing',grand_total)
   // console.log('tehdost');
    get_item_sum_with_tax();
    ...
    // var gross_sum = 0;
    // var net_discount = 0;
    // console.log('asdfasd-sf',_form_div_current.children());
    
    // $("td.tax-amount"+_data_pitch).each(function () {
    //     tax_sum += parseFloat($(this).text());
    // });
    // $("td.total-amount"+_data_pitch).each(function () {
    ...
    // //alert(gross_sum);
    // $("#display-total-tax-amount"+_data_pitch).text(tax_sum.toFixed(2));
    // $(_tax_amount).val(tax_sum.toFixed(2));

    // $("#display-grand-amount"+_data_pitch).text(grand_sum.toFixed(2));
    // $(_total_grand_amount).val(grand_sum.toFixed(2));
    
    // $(_total_gross_amount).val(gross_sum.toFixed(2));

    // $(_total_net_amount).val(net_discount.toFixed(2));
    // //$("#total_net_discount").text(net_discount.toFixed(2));

    // $("#display-gross-total-amount"+_data_pitch).text(gross_sum.toFixed(2));
    // $("#total_gross_sum"+_data_pitch).val(gross_sum.toFixed(2));
...
$("body").on("change", ".select-item-product", function () {
    ...
    .done(function (product) {
        ...
        // if($(el).not("select"))
        if(Global_client_gst == undefined ){
            Global_client_gst = $('#clientstate').text().trim();    
            //Global_client_gst = $('.cstate-div-default').text().trim();    
...
function get_item_sum_with_tax() {
    ...
    //$("#total_net_discount").text(net_discount.toFixed(2));
    //console.log('dasfsd',gross_sum);
    $('input[name="revised_quote"]').val(grand_sum.toFixed(2));
...
// change_billing_type
    ...
    function setGstDefaultValue(){
        let attr = $('.cstate-div-default').text().trim();
        console.log(attr);
        // debugger;
        if( attr  == 24 ){
